...
def SAVE_dATA( Cdata,RG_step,a_list,Dcut,name):
    ...
    dataframe.index=['L={}x{}'.format(2**(num_steps),2**(num_steps+1)) for num_steps in range(RG_step) ]
...
for T in T_list:
    ...
    SAVE_dATA(eigenvalues,1+trgstep,EigVnums,dcut,FILE+'unit1x2_T'+str(format( T, '.3f'))+'_D'+str(dcut)+'.csv' )
