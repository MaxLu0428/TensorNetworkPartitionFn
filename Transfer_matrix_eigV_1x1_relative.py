...
deltT = (4.0/step)
dcut = #bond_dim#
FILE = './transfer_matrix_eigenvalue_relative/'
FILE2 = './transfer_matrix_rescaleFactor_relative/'
...
for i,T in enumerate(T_list):
    ...
    for RG in range(trgstep):
        ...
        if len(Y) < EigVnums:
            ...
            Ys = list(Y)[::-1]
            eigenvalues[RG,:] = np.log(np.array(Ys + remains0))
        else:
            Ys = Y[:-1*EigVnums-1:-1]
            eigenvalues[RG,:] = np.log(Ys)
